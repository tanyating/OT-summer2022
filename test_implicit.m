...
%% start transport
z = x-mean(x)+mean(y); % start the transport with the original samples
w = z; % start with no free transformation

K = 500; % a threshold number of steps;
MAX_STEP = K + 200; % maximum steps of grad dc
...
lambda = 5e4; % intial regularization parameter 
...
% c = max(sqrt(sum((y-x).^2,2)));%8; % initial multiplier of bandwidth
% c = norm(mean(x)-mean(y))*max(mean(std(x)),mean(std(y)));
% c = max(8,c);
c = 8;
...
while (i<K) %&& norm(gradL)>tol)
    ...
    while (k<INNER_STEP && norm(gradL)>tol) % inner loop
        % 1) grad dc wrt z
        [z,zc,eta(i+2)] = imp2_grad_dc_z(w,y,z,zc,a,b,lambda,eta(i+1),eta_tol);
...
diag_blk = zeros(d,N*d);
for l1=1:d
    diag_blk(l1,l1:d:end) = 1/N + (-1/a(l1))*lambda.*tmp5...
        - (-1/b(l1))*lambda.*tmp6;
    for l2=1:d
        diag_blk(l1,l2:d:end) = diag_blk(l1,l2:d:end) + lambda*(c1.*sum(tmp1(l1,:,:).*tmp1(l2,:,:).*tmp3,3)...
            - c2.*sum(tmp2(l1,:,:).*tmp2(l2,:,:).*tmp4,3));
    end
end
diag_blk = mat2cell(diag_blk,[d],d.*ones(1,N));
res = sparse(blkdiag(diag_blk{:}));
% res(1:d,1:d)
% 
% diag_block = cell(N,1);
% % size(tmp3)
...
%     diag_block{i} = diag(diag_val)
%     
%     for j=1:N
% %         i,j
% %         tmp3(i,j)
%         diag_block{i} = diag_block{i} + lambda*c1.* kron(tmp1(:,i,j)',tmp1(:,i,j)).*tmp3(1,i,j);
%         diag_block{i} = diag_block{i} - lambda*c2.* kron(tmp2(:,i,j)',tmp2(:,i,j)).*tmp4(1,i,j);
%     end
%     
% end
% 
% res = blkdiag(diag_block{:});
% res(1:d,1:d)
...
function [znew] = pred_corr(x,y,z,c,a,b,lambda,eta)
...
eta = eta*2;
znew = pred_corr(x,y,z,c,a,b,lambda,eta);
cnew = znew;
[LFnew,gradLF_new] = grad_LF(y,znew,cnew,a,b);
[LCnew,gradLC_new] = grad_LC(x,znew);
...
% implicit grad dc wrt z
function [znew,cnew,eta] = imp2_grad_dc_z(x,y,z,c,a,b,lambda,eta,eta_tol)
...
eta = eta*2;
zst = pred_corr(x,y,z,c,a,b,lambda,eta);
[LFnew,gradLF_new] = grad_LF(y,zst,zst,a,b);
[LCnew,gradLC_new] = grad_LC(x,zst);
...
% while ((n2 >= n1) && (abs(n1-n2)>0.1)) % find reasonable learning rate (when the gradient is closer to 0)
    eta = eta/2;
    zst = pred(x,y,z,c,a,b,lambda,eta);
    [LFnew,gradLF_new] = grad_LF(y,zst,zst,a,b);
    [LCnew,gradLC_new] = grad_LC(x,zst);
    Lnew = LCnew + lambda.*LFnew;
...
znew = pred(x,y,z,zst,a,b,lambda,eta);
cnew = znew;
[LFnew,gradLF_new] = grad_LF(y,znew,cnew,a,b);
[LCnew,gradLC_new] = grad_LC(x,znew);
