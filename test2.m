...
%     y = mvnrnd([100;10],eye(2),M); % samples drawn from mu (M-by-d matrix)
    x = mvnrnd([1;1],[10 0;0 1],N); % samples drawn from rho (N-by-d matrix)
    theta = pi/2;
...
eta2 = 0.1; % initial (small) learning rate wrt free trans
eta3 = 0.1; % initial (small) learning rate wrt free rot
gamma = 300; % a threshold number of steps;
MAX_STEP = gamma + 200; % maximum steps of grad dc
...
while (i<gamma) %&& norm(gradL)>tol)
    ...
    while (k<INNER_STEP && norm(gradL)>tol) % inner loop
        % 1) grad dc wrt z
        [z,eta(i+2)] = grad_dc(w,y,z,a,b,lambda,eta(i+1),eta_tol);
...
% plot learning rates
figure();
plot(0:MAX_STEP,eta,'r.', 'Markersize', 3); hold on;
...
function [znew,eta] = grad_dc(x,y,z,a,b,lambda,eta,eta_tol)
